...
def newest_age_minutes(df: pd.DataFrame, *columns: str) -> float | None:
    ...
    for col in columns:
        if col in df.columns:
            ...
            if not vals.empty:
                # Do not interpret future day-ahead values as negative age.
...
def render_dashboard():
    ...
    with c1:
        ...
        st.caption(f"Build {APP_BUILD_VERSION} • source health: green / yellow / red • Elering actual-only")
    ...
    with st.expander("🔌 Andmeallikate staatus — roheline / kollane / punane", expanded=False):
        st.caption("🟢 töötab ja andmed on piisavalt värsked · 🟡 töötab, kuid andmed on vanad/osalised/fallback · 🔴 päring või autentimine ebaõnnestus")

        left_status, right_status = st.columns(2)

        # Operational freshness thresholds are deliberately source-specific.
        price_has_current = any(v is not None for v in current_prices.values())
        sys_age_min = age_minutes(sys_time)
        gen_age_min = newest_age_minutes(entsoe_generation, "time_local", "time_utc")
        load_age_min = newest_age_minutes(entsoe_load, "time_local", "time_utc")

        with left_status:
            source_badge(
                "Elering hinnad",
                detail=status_detail(price_status, extra=("käimasolev MTU olemas" if price_has_current else "käimasolev MTU puudub")),
                level=status_level(price_status, has_data=(not prices.empty and price_has_current)),
            )
            source_badge(
                "Elering süsteem",
                detail=status_detail(system_status, age_text=fmt_age(sys_time)),
                level=status_level(system_status, has_data=not system_df.empty, age_min=sys_age_min, warn_after_min=15),
            )

            umm_level = "error" if bool(umm_meta.error) else ("warning" if not umm_rows else "ok")
            source_badge(
                "Nord Pool UMM",
            ...
            storage_age = newest_age_minutes(storage_df, "gasDayStart", "gasDay", "date")
            source_badge(
                "GIE AGSI+",
                detail=status_detail(storage_status, age_text=(f"{int(storage_age//1440)} p" if storage_age is not None and storage_age >= 1440 else (f"{int(storage_age//60)} h" if storage_age is not None else None))),
                level=status_level(storage_status, has_data=not storage_df.empty, age_min=storage_age, warn_after_min=72*60),
            )

            source_badge(
                entsoe_generation_status.source or "ENTSO-E tootmine",
                detail=status_detail(entsoe_generation_status, age_text=(fmt_age(entsoe_generation["time_local"].max()) if not entsoe_generation.empty and "time_local" in entsoe_generation else None)),
                level=status_level(entsoe_generation_status, has_data=not entsoe_generation.empty, age_min=gen_age_min, warn_after_min=120),
            )
            source_badge(
                entsoe_load_status.source or "ENTSO-E tarbimine",
                detail=status_detail(entsoe_load_status, age_text=(fmt_age(entsoe_load["time_local"].max()) if not entsoe_load.empty and "time_local" in entsoe_load else None)),
                level=status_level(entsoe_load_status, has_data=not entsoe_load.empty, age_min=load_age_min, warn_after_min=120),
            )
        ...
        with right_status:
            for area, (df_ngp, st_ngp) in ngp_current_results.items():
                source_badge(
                    f"EEX {area} NGP",
                    detail=status_detail(st_ngp, extra=(f"{len(df_ngp)} väärtust" if not df_ngp.empty else "väärtus puudub")),
            ...
            source_badge(
                "EIA Brent",
                detail=status_detail(brent_status, age_text=(fmt_age(brent_df["Date"].max()) if not brent_df.empty and "Date" in brent_df else None)),
                level=status_level(brent_status, has_data=not brent_df.empty, age_min=newest_age_minutes(brent_df, "Date"), warn_after_min=7*24*60),
            )
            source_badge(
                "EEX EUA oksjon",
                detail=status_detail(eua_status, age_text=(fmt_age(eua_df["Date"].max()) if not eua_df.empty and "Date" in eua_df else None)),
                level=status_level(eua_status, has_data=not eua_df.empty, age_min=newest_age_minutes(eua_df, "Date"), warn_after_min=14*24*60),
            )

            # Reserve and balancing sources: aggregate per market/region so the panel stays readable.
            for region, (rdf, statuses) in reserve_results.items():
                ...
                errors = [s.error or s.note for s in statuses if (s.error or s.note)]
                source_badge(f"BTD reservvõimsus {region}", detail=("; ".join(errors[:2]) if errors else f"{len(rdf)} rida"), level=level)
            ...
            for region, (edf, statuses) in balancing_energy_results.items():
                ...
                errors = [s.error or s.note for s in statuses if (s.error or s.note)]
                source_badge(f"BTD balancing energy {region}", detail=("; ".join(errors[:2]) if errors else f"{len(edf)} rida"), level=level)

            flow_ok = sum(1 for s in entsoe_flow_statuses if s.ok)
            flow_level = "error" if flow_ok == 0 else ("warning" if flow_ok < len(entsoe_flow_statuses) or entsoe_flows.empty else "ok")
            source_badge("ENTSO-E füüsilised vood", detail=f"{flow_ok}/{len(entsoe_flow_statuses)} päringut OK", level=flow_level)

            ntc_ok = sum(1 for s in entsoe_ntc_statuses if s.ok)
            ntc_level = "error" if ntc_ok == 0 else ("warning" if ntc_ok < len(entsoe_ntc_statuses) or entsoe_ntc.empty else "ok")
            source_badge("ENTSO-E päev-ette NTC", detail=f"{ntc_ok}/{len(entsoe_ntc_statuses)} päringut OK", level=ntc_level)
